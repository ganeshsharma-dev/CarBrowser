...
private fun handleCleartextIfNeeded(view: WebView, uri: Uri?, callbacks: BrowserCallbacks, onPageStart: Boolean = false): Boolean {
    ...
    if (com.car.browser.data.BrowserPreferences.isHostAllowedCleartext(view.context, host)) {
        return false
    }
    ...
    val allowHost = {
        view.context?.let { ctx ->
            ...
            if (hostToStore != null) com.car.browser.data.BrowserPreferences.addAllowedCleartextHost(ctx, hostToStore)
        }
        view.setTag(R.id.webview_allow_once_uri_tag, uri.toString())
        view.post { view.loadUrl(uri.toString()) }
        kotlin.Unit
...
private fun WebView.applyBrowserIdentity(profile: UserAgentProfile, desktop: Boolean) {
    ...
    val scalePercent = com.car.browser.data.BrowserPreferences.getGlobalScalePercent(context)
    if (desktop) {
        setInitialScale(0)
...
private const val MOBILE_CHROME_UA = "Mozilla/5.0 (Linux; Android 14) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${CHROME_VERSION} Mobile Safari/537.36"
