...
import com.car.browser.ui.adapters.TabAdapter
import com.car.browser.web.BrowserCallbacks
import com.car.browser.web.configureWebView
import com.car.browser.web.releaseCompletely
...
data class BrowserTab(
    ...
    val speechBridge: com.car.browser.web.SpeechRecognitionBridge,
...
class TabManager(
    ...
    fun initializeTabs(
        ...
        // 1. Restore from Memory (if Activity was just recreated)
        if (savedInstanceState != null) {
            val tabStates = savedInstanceState.getBundle("browser_tab_states")
            if (tabStates != null) {
                browserTabs.forEach { tab ->
                    val s = tabStates.getBundle("tab_${tab.id}")
                    if (s != null) {
                        tab.webView.restoreState(s)
                    }
                }
            }
        } 
        // 2. Restore from Disk (if App was completely restarted)
        else if (shouldRestoreSavedTabs) {
            browserTabs.forEach { tab ->
                restoreTabStateFromDisk(tab)
            }
        }
    }
    ...
    fun createBrowserTab(
        ...
        val speechBridge = com.car.browser.web.SpeechRecognitionBridge(tabView) { pageUrl ->
            callbacks.requestSpeechRecognitionMicrophoneAccess(tab.id, pageUrl)
        }
        
        tab = BrowserTab(
            id = nextTabId++,
            persistentId = persistentId ?: java.util.UUID.randomUUID().toString(),
            webView = tabView,
    ...
    private fun setupWebMessageListener(webView: android.webkit.WebView, speechBridge: com.car.browser.web.SpeechRecognitionBridge) {
        if (WebViewFeature.isFeatureSupported(WebViewFeature.WEB_MESSAGE_LISTENER)) {
            WebViewCompat.addWebMessageListener(webView, com.car.browser.web.SpeechRecognitionBridge.BRIDGE_OBJECT_NAME, setOf("*")) { webViewInstance, message, sourceOrigin, isMainFrame, _ ->
    ...
    fun saveState(outState: android.os.Bundle) {
        val tabStates = android.os.Bundle()
        browserTabs.forEach { tab ->
            val s = android.os.Bundle()
            tab.webView.saveState(s)
            tabStates.putBundle("tab_${tab.id}", s)
        }
        outState.putBundle("browser_tab_states", tabStates)
    }

    fun persistTabSession() {
        val entries = browserTabs.map { 
            BrowserPreferences.TabSessionEntry(
                it.currentUrl.takeIf { u -> u.isNotBlank() }, 
                it.currentTitle.takeIf { t -> t.isNotBlank() },
                it.persistentId
            ) 
        }
        ...
        // Also save current tab's binary state to disk
        activeTab?.let { saveTabStateToDisk(it) }
    }

    private fun saveTabStateToDisk(tab: BrowserTab) {
        try {
            val bundle = android.os.Bundle()
            tab.webView.saveState(bundle)
            val file = java.io.File(activity.cacheDir, "tab_state_${tab.persistentId}.bin")
    ...
    private fun restoreTabStateFromDisk(tab: BrowserTab) {
        try {
            val file = java.io.File(activity.cacheDir, "tab_state_${tab.persistentId}.bin")
            if (file.exists()) {
                ...
                val bundle = parcel.readBundle(activity.classLoader)
                parcel.recycle()
                if (bundle != null) {
                    tab.webView.restoreState(bundle)
                }
            }
        } catch (e: Exception) {
            e.printStackTrace()
