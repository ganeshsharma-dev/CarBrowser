...
import com.car.browser.model.AppThemeMode
        ...
        return runCatching {
            ...
            buildList(array.length()) {
                for (index in 0 until array.length()) {
                    ...
                    val pId = entry.optString("persistentId").trim().takeIf { it.isNotEmpty() }
                    if (normalizedUrl != null) {
                        add(TabSessionEntry(url = normalizedUrl.ifEmpty { null }, title = title, persistentId = pId))
                    }
                }
            }.take(MAX_OPEN_TABS)
        ...
        normalizedTabs.forEach { tab ->
            array.put(
                JSONObject().apply {
                    ...
                    put("persistentId", tab.persistentId.orEmpty())
                }
            )
        }
